...
class SCRFD:
    ...
    def detect(self, img, thresh=0.5, input_size=None, max_num=0, metric='default'):
        ...
        inputs.append(
            tritongrpcclient.InferInput("INPUT_PREPROCESS_FACE", data.shape, "UINT8"))
        inputs[0].set_data_from_numpy(data)
        results = self.triton_client.infer(model_name="preprocess_detect",
                                           inputs=inputs,
                                           outputs=outputs)
        det_scale = results.as_numpy("OUTPUT_SCALES")[0][0]
        blobs = results.as_numpy("OUTPUT_PREPROCESS_FACE")

        inputs = []
        outputs = []
        data = np.expand_dims(img, axis=0)
        inputs.append(
            tritongrpcclient.InferInput("input.1", blobs.shape, "FP32"))
        inputs[0].set_data_from_numpy(blobs)

        results = self.triton_client.infer(model_name="inference_detect",
                                           inputs=inputs,
                                           outputs=outputs)
        
        score_8 = results.as_numpy("score_8")

        

        # print(img.shape)
        # blob,det_scale=self.preprocess(img,input_size)
        # print(blob.shape)
        scores_list, bboxes_list, kpss_list = self.forward(blobs, thresh)
        p
...
if __name__ == '__main__':
    ...
    detector = SCRFD(model_file='./scrfd_2.5g_bnkps.onnx')
    detector.prepare(-1)
    img_paths = ['test5.jpg']
