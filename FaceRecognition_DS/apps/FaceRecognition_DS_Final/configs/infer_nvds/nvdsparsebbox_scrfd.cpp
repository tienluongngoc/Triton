    ...
    std::vector<NvDsInferInstanceMaskInfo> &objectList)
{
  ...
  for (int j = 0; j < predictedBoxes.size(); j++)
  {
    ...
    object.left = predictedBoxes[j].x1;
    object.top = predictedBoxes[j].y1;
    object.width = predictedBoxes[j].x2 - predictedBoxes[j].x1;
    object.height = predictedBoxes[j].y2 - predictedBoxes[j].y1;
    ...
    landmark_box[11] = predictedBoxes[j].x1;
    landmark_box[10] = predictedBoxes[j].y1;
    landmark_box[13] = predictedBoxes[j].x2;
    landmark_box[12] = predictedBoxes[j].y2;
